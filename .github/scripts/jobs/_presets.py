...
tiny / full model. `tiny:<Arch>` (or a preset tiny of "auto") resolves through tiny_models.py,
which builds a tiny random copy locally when no tiny repo exists.
"""
...
def tiny_model(key):
    """The tiny test repo for an architecture / model_type / model id (tiny_models.py next to jobs/),
    else a tiny random copy built locally (tiny_make.py); LookupError when neither works."""
    import sys
    ...
    import tiny_models
    return tiny_models.get(key)


def resolve(presets, name, tiny=False, model=None):
    """(preset dict copy, model id). Unknown name -> KeyError listing the choices.

    ``--model tiny:<key>`` picks the tiny test repo for an architecture, model_type or model id
    (``tiny:DeepseekV4ForCausalLM``), and a preset with ``tiny="auto"`` derives its tiny model
    from its full ``model``."""
    if name not in presets:
        raise KeyError(f"unknown preset {name!r}; choose from {sorted(presets)}")
    p = dict(presets[name])
    chosen = model or (p["tiny"] if tiny else p["model"])
    if chosen == "auto":
        chosen = tiny_model(p["model"])
    elif isinstance(chosen, str) and chosen.startswith("tiny:"):
        chosen = tiny_model(chosen[len("tiny:"):])
    return p, chosen
