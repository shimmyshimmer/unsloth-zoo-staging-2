...
Base runs first. When the base arm alone decides the verdict (it crashed, failed its own checks or
wrote no metrics: compare.py would say VOID / NOT_RUN whatever head does), the head arm is not run.
Never with --expect-fail-on-base.
...
def base_early_exit(base_arm):
    """(verdict, reason) when the base arm alone decides compare.py's verdict (a VOID / NOT_RUN
    whatever head does: base crashed, failed its own checks, wrote no metrics), else None. The head
    arm is then not run: on a broken Core environment that halves the wasted time, and the verdict
    is the one compare.py would print. Never with --expect-fail-on-base (base failing is the point)."""
    import compare
    if "void" in base_arm:
        return "VOID", base_arm["void"]
    try:
        base = json.loads(Path(base_arm["out"]).read_text())
    except (OSError, ValueError):
        base = None
    return compare.base_decides(base)
...
def main(argv=None):
    ...
    try:
        ...
        try:
            ...
            for arm, wt in (("base", base_wt), ("head", head_wt)):
                arms.append(run_arm(arm, wt, a.python, module, script, args, outdir))
                if arm == "base" and not a.expect_fail_on_base:
                    early = base_early_exit(arms[0])
                    if early:
                        _log(f"base arm decides the verdict ({early[0]}: {early[1]}); head arm not run")
                        break
        finally:
    ...
    if early and not void:
        result.update(verdict=early[0], reason=early[1], head_skipped=True)
        (outdir / "head.json").unlink(missing_ok=True)   # never compare against a stale head arm
        rc, text = 2, f"VERDICT {early[0]} {early[1]} (head arm not run)"
    elif void:
