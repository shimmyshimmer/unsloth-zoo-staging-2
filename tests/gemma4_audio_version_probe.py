"""Does Gemma 4 audio training actually work on the installed mlx-vlm?

The audio family gate pins gemma4 to mlx-vlm 0.4.4, the version its probes ran
on. Its processor changed in 0.5.0, upstream reports it failing to load from
0.6.4 (Blaizzy/mlx-vlm#1526, 0.6.3 good), and 0.6.3 added a separate
`gemma4_unified` family. Whether 0.5.0 through 0.6.4 genuinely work has never
been measured, so the pin is conservative rather than evidenced.

This probe measures it, on real Apple Silicon against a real checkpoint. It
deliberately opens the gate for whatever mlx-vlm is installed, because the
...
gets wrong on the way (PR 879 undoes mlx-vlm's double transpose of the audio
convs on pre-converted checkpoints). Probing raw mlx-vlm would report failures
zoo does not actually have.
